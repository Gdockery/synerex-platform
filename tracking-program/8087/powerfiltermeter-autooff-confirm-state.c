...
int main(int argc, char *argv[]){
	...
	if(modbus_read_input_registers(sdm630, 0x0000, 79, catcher)>0) {
		...
		if ( catcher[42] < 15 ) { //whatever the pf was told to do, it is actually off
               	    printf("\"pfstate\": \"OFF\", ");  //off
		    remove("/home/pi/ON");
		    fp = fopen("/home/pi/OFF", "w"); // "w" mode creates the file
		} else {
               	    printf("\"pfstate\": \"ON\", ");  //on
		    remove("/home/pi/OFF");
		    fp = fopen("/home/pi/ON", "w"); // "w" mode creates the file
		}
