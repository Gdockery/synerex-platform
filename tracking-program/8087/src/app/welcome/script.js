...
(function () {
	var userAgent = navigator.userAgent.toLowerCase(),
		...
		plugins = {
			...
			ticker:                  document.querySelectorAll( '.ticker-data' ),
	...
	$(function () {
		...
		// ticker get data from file
		if ( plugins.ticker) {
			let start;
			let counters = [];
			function getTickerInit() {
				alert ('called getTickerInit');
			    $.ajax({
			        url: './ticker.txt',
			        dataType: 'text',

			        success: function(text) {
			        	let to = text.split(',').map(x=> parseInt(x));
			        	start = text.split(',').map(x=> parseInt(x));
			        	let counter;
			        	for ( var i = 0; i < plugins.ticker.length; i++ ) {

							start[i] = start[i] - (100 - (10 * Math.floor(Math.random() * 9)));
			        		let node = plugins.ticker[i];
							counter = aCounter({
								node: node,
								from: start[i],
								to: to[i],
			...
			function getLog() {
			    ...
			    $.ajax({
			        ...
			        success: function(text) {
			        	text = text.split(',').map(x=> parseInt(x));
			        
			        
			        	/*$("#ticker-peak-savings").text(text.peakSavings);
			            $("#ticker-peak-savings-amt").text(text.peakSavingsAmount.toLocaleString());
			            $("#ticker-kwh-savings").text(text.kwhSavings.toLocaleString());
			            $("#ticker-kwh-savings-amt").text(text.kwhSavingsAmount.toLocaleString());
			            $("#ticker-carbon-savings").text(text.carbonSavings.toLocaleString());
			            $("#ticker-carbon-savings-amt").text(text.carbonSavingsAmount.toLocaleString());
			            $("#ticker-i2r-savings").text(text.I2RLossSavings.toLocaleString());
			            $("#ticker-i2r-savings-amt").text(text.I2RLossSavingsAmount.toLocaleString());
			            $("#ticker-total-savings").text(text.totalSavings.toLocaleString());*/
			         
			            
			        	for ( var i = 0; i < counters.length; i++ ) {
							counters[i].params.from = start[i];
							counters[i].params.to = text[i];
							counters[i].run();
			            ...
			            setTimeout(getLog, 60000); // refresh every 30 seconds
			        }, fail: function() {
			        	alert('error 2');
			        }
			    })
