...
@Component({
  selector: 'energy-savings-form',
  template: `
    <form [formGroup]="form" (ngSubmit)="submit()">
      <div class="col-md-3">
        <div><strong>GENERATE ENERGY REPORT</strong></div>
        <div>Choose a start and end date to generate report.</div>
      </div>
      <div class="col-md-3">
      ...
      <div class="col-md-3">
        <div class="form-group">
          ...
          <div *ngIf="form.controls.toDate.hasError('invalidAfterDateField') && (form.controls.toDate.dirty || form.controls.toDate.touched)" class="alert alert-danger">
            To date must be before from date
          </div>
        </div>
      </div>
      <div class="col-md-3" style="padding-top: 22px">
        <button class="default-button green-button">Submit</button>
      </div>
    </form>
  `
})
export class EnergySavingsFormComponent implements OnInit {
  ...
  getFormData() {
    return {
      fromDate: this.timeHelpers.getMomentFromDatepickerDictionary(this.form.value.fromDate.date).valueOf(),
