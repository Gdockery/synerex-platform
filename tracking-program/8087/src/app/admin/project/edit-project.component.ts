...
export class ProjectEditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.form.addControl('numberOfMeters', new FormControl('', []));
  ...
  fetch() {
    ...
    this.projectService.get(this.selectedProjectId).subscribe(currentProjectData => {
      ...
      if(this.project.subStartDate) {
        this.form.patchValue({subStartDate: this.timeHelpers.getDatepickerDictionaryFromString(this.project.subStartDate, 'YYYY-MM-DD')});
      }
  ...
  submitEditProjectForm() {
    ...
    if(formData.reportFields.paymentPlan == '2') {
