...
export class EditSwitchComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute, private switchService: SwitchesService, private confirmationService: ConfirmationService, private userService: CurrentUserService) {
    this.switchId = route.snapshot.params['id'];
  }

  ngOnInit() {
    this.switchService.get(this.switchId).subscribe((switchModel:any) => {
      this.switch = switchModel.response;
    });
  }

  update(params) {
    this.switchService.update(this.switchId, {valuesToSet: params}).subscribe(response => {
  ...
  confirmRetire() {
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.switchService.remove(this.switchId).subscribe(response => {
          this.retired = true;
          this.renaming = false;
          this.changingNetwork = false;
        })
