module.exports = {
  ...
  description: 'Update an existing switch alert group.',


  inputs: {

    id: {
      description: 'The ID of the switch alert group to update.',
      example: 123,
      required: true
    },

    switches: {
      description: 'IDs of switches to add to the alert.',
      extendedDescription: 'This list, if provided, will completely replace the previous list of switches attached to this alert.',
      example: [1]
    },
  ...
  fn: function (inputs, exits) {
    ...
    sails.helpers.web.alerts.updateAlert(_.extend({ deviceType: 'switch', devices: inputs.switches, user: req.user }, _.omit(inputs, 'switches'))).exec({
      ...
      error: exits.error,
      success: function() {
        sails.helpers.web.alerts.getAlertDetails({ deviceType: 'switch', user: req.user, id: inputs.id }).exec(exits);
      }
    });
