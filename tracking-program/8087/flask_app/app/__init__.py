...
def create_app(config_class=Config):
    ...
    @app.cli.command("compute-interval-baseline")
    @click.option("--project-id", required=True, type=int, help="Project ID")
    @click.option("--from-date", required=True, help="Baseline period start (YYYY-MM-DD), UTC")
    @click.option("--to-date",   required=True, help="Baseline period end   (YYYY-MM-DD), UTC")
    @click.option("--min-kw",    default=10.0,  type=float, show_default=True,
                  help="Min kW to include (filters out meter-offline rows)")
    @click.option("--max-kw",    default=50000.0, type=float, show_default=True,
                  help="Max kW to include (filters out sensor spikes)")
    def compute_interval_baseline(project_id, from_date, to_date, min_kw, max_kw):
        """
        Build 96-slot interval baseline for a project from its isMain=1 meter.

        Reads meterdata for the given date range (when Synerex units were off /
        ...
        summary = _db.session.execute(
            _text("""
                SELECT ROUND(MIN(avg_kw),0), ROUND(MAX(avg_kw),0),
                       ROUND(AVG(avg_kw),0), ROUND(AVG(avg_pf),4),
                       ROUND(AVG(avg_thd),2), MIN(sample_count)
                FROM baseline_intervals WHERE project_id=:pid
            """),
            {"pid": project_id},
        ).fetchone()
        print(f"[interval-baseline] DONE — {upserted} slots upserted")
        print(f"[interval-baseline] kW range: {summary[0]}–{summary[1]}  avg: {summary[2]}")
        print(f"[interval-baseline] avg PF: {summary[3]}  avg THD: {summary[4]}%")
        print(f"[interval-baseline] min samples per slot: {summary[5]}")
        print(f"\n[interval-baseline] Run next: flask backfill-analytics --project-id {project_id}")
