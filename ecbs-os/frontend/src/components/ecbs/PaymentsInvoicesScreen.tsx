      ...
      <div className="grid h-full grid-cols-[144px_1fr]">
        ...
        <main className="flex min-w-0 flex-col bg-[radial-gradient(circle_at_top_right,rgba(0,220,255,.1),transparent_30%),linear-gradient(180deg,#04111c,#020910)] px-4">
          ...
          <section className="grid h-[86px] grid-cols-5 gap-2">
            <PaymentKpi label="Total Invoiced" value="No Data" detail="source_missing" icon="▧" tone="#147dff" />
            <PaymentKpi label="Total Paid" value="No Data" detail="source_missing" icon="▤" tone="#05ff5e" />
            <PaymentKpi label="Outstanding Balance" value="No Data" detail="source_missing" icon="♙" tone="#f59e0b" />
            <PaymentKpi label="Overdue Amount" value="No Data" detail="source_missing" icon="△" tone="#ef4444" />
            <PaymentKpi label="Paid This Month" value="No Data" detail="source_missing" icon="▣" tone="#8b5cf6" />
          </section>
...
function PaymentsSidebar() {
  ...
  return <aside className="relative h-full overflow-hidden border-r border-cyan-300/10 bg-[#030c15] px-3 py-3"><div className="mb-3"><div className="text-[29px] font-black italic leading-none tracking-[-0.12em]"><span className="text-[#05ff5e]">X</span>ECO</div><div className="mt-1 text-[8px] font-bold uppercase tracking-[0.48em] text-[#05ff5e]">Energy</div></div><nav className="space-y-2 text-[8px]">{groups.map(([title,items])=><section key={String(title)}><h2 className="mb-1 text-[#05ff5e]">{title}</h2>{(items as string[]).map(item=><div className={item==="Payments" ? "flex h-[20px] items-center rounded bg-[#063b27] px-1.5 text-[#05ff5e]" : "flex h-[20px] items-center rounded px-1.5 text-slate-300"} key={item}>⌘ &nbsp; {item}{item==="Alarms & Events"?<b className="ml-auto grid size-4 place-items-center rounded-full bg-red-500 text-[8px] text-white">0</b>:null}</div>)}</section>)}</nav><div className="absolute bottom-[44px] left-3 right-3 rounded border border-cyan-300/12 bg-[#061421] p-2 text-[8px]"><div className="text-white">☏ Need Help?</div><div className="text-slate-400">Contact Support</div></div><div className="absolute bottom-2 left-3 right-3 text-[7px] text-slate-500">© 2025 XECO Energy Corporation.</div></aside>;
}
