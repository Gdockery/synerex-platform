...
export default async function DeploymentClosureConfirmationPage({ params }: PageProps) {
  const { deploymentId } = await params;
  const data = await getDeploymentCompletionDataFromApi(deploymentId);

  return <DeploymentWorkflowScreen data={data} deploymentId={deploymentId} variant="closure" />;
}
