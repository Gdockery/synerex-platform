...
export default async function CustomerAcceptancePage({ params }: PageProps) {
  const { deploymentId } = await params;
  const data = await getDeploymentCompletionDataFromApi(deploymentId);

  return <DeploymentWorkflowScreen data={data} deploymentId={deploymentId} variant="acceptance" />;
}
