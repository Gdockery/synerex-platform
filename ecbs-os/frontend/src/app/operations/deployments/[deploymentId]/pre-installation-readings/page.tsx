...
export default async function PreInstallationReadingsPage({ params }: PageProps) {
  const { deploymentId } = await params;
  const fieldData = await getDeploymentFieldWorkflowDataFromApi(deploymentId);

  return <DeploymentWorkflowScreen deploymentId={deploymentId} fieldData={fieldData} variant="preReadings" />;
}
