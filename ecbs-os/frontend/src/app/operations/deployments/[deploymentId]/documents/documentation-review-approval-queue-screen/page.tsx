...
export default async function DocumentationReviewApprovalQueuePage({ params }: PageProps) {
  const { deploymentId } = await params;
  const documentationData = await getDeploymentDocumentationDataFromApi(deploymentId);

  return <DeploymentWorkflowScreen deploymentId={deploymentId} documentationData={documentationData} variant="reviewQueue" />;
}
