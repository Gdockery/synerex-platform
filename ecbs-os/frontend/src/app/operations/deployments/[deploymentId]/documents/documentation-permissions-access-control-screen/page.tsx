...
export default async function DocumentationPermissionsAccessControlPage({ params }: PageProps) {
  const { deploymentId } = await params;
  const documentationData = await getDeploymentDocumentationDataFromApi(deploymentId);

  return <DeploymentWorkflowScreen deploymentId={deploymentId} documentationData={documentationData} variant="permissions" />;
}
