...
export default async function DocumentationVersionHistoryPage({ params }: PageProps) {
  const { deploymentId } = await params;
  const documentationData = await getDeploymentDocumentationDataFromApi(deploymentId);

  return <DeploymentWorkflowScreen deploymentId={deploymentId} documentationData={documentationData} variant="versionHistory" />;
}
