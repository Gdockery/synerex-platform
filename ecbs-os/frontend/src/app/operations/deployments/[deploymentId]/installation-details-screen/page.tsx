...
export default async function InstallationDetailsPage({ params }: PageProps) {
  const { deploymentId } = await params;
  const fieldData = await getDeploymentFieldWorkflowDataFromApi(deploymentId);

  return <DeploymentWorkflowScreen deploymentId={deploymentId} fieldData={fieldData} variant="installationDetails" />;
}
