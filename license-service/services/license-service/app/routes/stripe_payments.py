...
def _issue_license_for_stripe(db: Session, order: BillingOrder, intent: dict) -> None:
    ...
    # For renewal orders — link old and new licenses together
    if order.order_id.startswith("ORD-RENEW-") and order.notes:
        _link_renewal_licenses(db, order, license_rec)
...
# ---------------------------------------------------------------------------
# Renewal linkage helper
# ---------------------------------------------------------------------------

def _link_renewal_licenses(db, order: BillingOrder, new_license) -> None:
    """For renewal orders, link the new license back to the previous one.

    The renewal order's notes field carries a structured tag written by the
    client renewal endpoint:  RENEWAL_OF:<old_license_id>|AUTO_RENEW:<true|false>
    """
    from ..models.license import License as _License

    notes = order.notes or ""
    old_license_id: Optional[str] = None
    wants_auto_renew = False

    for part in notes.split("|"):
        part = part.strip()
        if part.startswith("RENEWAL_OF:"):
            old_license_id = part[len("RENEWAL_OF:"):].strip()
        elif part.startswith("AUTO_RENEW:"):
            wants_auto_renew = part[len("AUTO_RENEW:"):].strip().lower() == "true"

    if not old_license_id:
        return

    old_lic = db.get(_License, old_license_id)
    if not old_lic:
        logger.warning("[stripe] Renewal linkage: old license %s not found", old_license_id)
        return

    # Cross-link old ↔ new
    old_lic.renewal_license_id = new_license.license_id
    new_license.previous_license_id = old_license_id
    new_license.auto_renew = wants_auto_renew
    db.commit()
    logger.info("[stripe] Linked renewal: %s → %s (auto_renew=%s)",
                old_license_id, new_license.license_id, wants_auto_renew)
