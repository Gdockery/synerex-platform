...
class Payment(Base):
    ...
    id: Mapped[int] = mapped_column(String(255), primary_key=True)  # Payment ID from gateway
    order_id: Mapped[str] = mapped_column(String(255), nullable=False, index=True)
    org_id: Mapped[str] = mapped_column(String(255), nullable=False, index=True)
    amount: Mapped[str] = mapped_column(String(255), nullable=False)  # Decimal as string
    currency: Mapped[str] = mapped_column(String(255), default="USD")
    gateway: Mapped[str] = mapped_column(String(255), nullable=False)  # "stripe", "paypal", etc.
    gateway_transaction_id: Mapped[str | None] = mapped_column(String(255), nullable=True)
    status: Mapped[str] = mapped_column(String(255), nullable=False)  # "pending", "completed", "failed", "refunded"
    payment_method: Mapped[str | None] = mapped_column(String(255), nullable=True)  # "card", "paypal", etc.
...
class Invoice(Base):
    ...
    currency: Mapped[str] = mapped_column(String(255), default="USD")
    status: Mapped[str] = mapped_column(String(255), nullable=False)  # "draft", "sent", "paid", "overdue", "cancelled"
