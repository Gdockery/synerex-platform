...
class BillingOrder(Base):
    ...
    plan: Mapped[str] = mapped_column(String(255), nullable=False)        # baseline|pro|enterprise etc.

    term_start: Mapped[str] = mapped_column(String(255), nullable=False)  # YYYY-MM-DD
    term_end: Mapped[str] = mapped_column(String(255), nullable=False)    # YYYY-MM-DD

    seat_count: Mapped[int] = mapped_column(Integer, default=0)
    meter_count: Mapped[int] = mapped_column(Integer, default=0)

    unit_price: Mapped[str] = mapped_column(String(255), default="0")     # string for simplicity
    amount_total: Mapped[str] = mapped_column(String(255), default="0")
    currency: Mapped[str] = mapped_column(String(255), default="USD")

    status: Mapped[str] = mapped_column(String(255), default="pending")   # pending|paid|overdue|cancelled
    ...
    agreement_version: Mapped[str | None] = mapped_column(String(255), nullable=True)  # e.g., "2026.01"
    agreement_accepted_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    agreement_accepted_by: Mapped[str | None] = mapped_column(String(255), nullable=True)  # email or org_id